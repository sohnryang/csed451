...
out vec3 diffuse_product_frag;
out float mat_shininess_frag;
out vec3 specular_product_frag;
...
void main() {
  ...
  light_direction_frag = normalize(light_pos - pos_modelview.xyz);
  eye_frag = normalize(-pos_modelview.xyz);
  ambient_frag = ambient_intensity * mat_ambient;
  diffuse_product_frag = diffuse_intensity * mat_diffuse;
  mat_shininess_frag = mat_shininess;
  specular_product_frag = specular_intensity * mat_specular;
...
in vec3 diffuse_product_frag;
in float mat_shininess_frag;
in vec3 specular_product_frag;
...
void main() {
  ...
  vec3 diffuse = max(dot(light_direction, transformed_normal), 0.0) * diffuse_product_frag;
...
int main(int argc, char **argv) {
  const std::string filename = "rooster.obj";
  ...
  std::uint8_t *texture_data =
      stbi_load("rooster_texture.jpg", &width, &height, &channel_count, 0);
